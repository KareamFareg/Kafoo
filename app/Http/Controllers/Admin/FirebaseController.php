...
$firebase = (new Factory)
     ->withServiceAccount(base_path() . '/kafoo-2a7a1-firebase-adminsdk-ht8n2-120c062d88.json')
     ->withDatabaseUri('https://kafoo-2a7a1-default-rtdb.firebaseio.com/');
...
$database = $firebase->createDatabase();
        ...
        $items = $database->getReference('Orders/' . $request->id);
        ...
        if (!empty($items)) {
            ...
            foreach ($items as $item) {
                if ($first < 1) {
                    $chaters['receiver_name'] = optional(User::where('id', $item['reciverid'])->select('id', 'name')->first())->name ?? __('words.deleted');
                    $chaters['sender_name'] = optional(User::where('id', $item['senderid'])->select('id', 'name')->first())->name ?? __('words.deleted');
                    $chaters_ids['receiver_id'] = $item['reciverid'];
...
$database = $firebase->createDatabase();
        ...
        // $items = $items->getvalue();
        echo"<pre>";
        var_dump($database->getReference('Chat')->getvalue());
        echo"</pre>";
        // return view('util.chat-users-admin', compact(['user', 'items']));
