...
class NotificationsController extends Controller
{
    ...
    public function notifyAll(Request $request)
    {
        ...
        // $request->flash();
        // return back();
        if($request->group == 'clients'){
            $request->group = 2;
        }elseif($request->group == 'shippers'){
            $request->group = 3;
        }
       $users = User::where('type_id',$request->group)->get();
       foreach($users as $user){
            $msg['user_sender'] = 0;
            $msg['user_reciever'] = $user->id;
            $msg['order'] = '';
            $msg['type'] = 10;
            ...
            $this->sendGCM($msg ,$user->fcm_token);

            $data['user_sender_id'] = 0;
            $data['user_reciever_id'] =$user->id;
            $data['table_name'] = '';
            $data['table_id'] = 0;
