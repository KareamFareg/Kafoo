...
class Order extends Model
{
    ...
    protected $fillable = [
        'id','status', 'note', 'invoice', 'shipper_id','price','deserve_more','comment','payment_type','payment_status','charge_wallet','canceld_by','tawseelMessage','refrenceCode',
        'cancel_date','coupon','discount','accept_date','delivery_date','commission_status','shipping','commission','shop_name','place','package_type','user_static','shipper_static','addition_service','addition_service_cost'
    ];
    ...
    {
        return optional($this->hasOne('App\Models\CategoryInfo', 'category_id', 'type')->where('language', app()->getLocale())->first())->image;
    }
