...
class NotificationService
{
    ...
    public function notifyOrder($to = [], $params = [])
    {
        ...
        foreach ($users as $user) {
            ...
            // get current item
            $order = Order::where('id', $params['order_id'])->select('id', 'user_id', 'fcm_token', 'code')->first();
            // get user sender
    ...
    public function notifyOrderAccepted($to = [], $params = [])
    {
        ...
        if (in_array('fcm', $to)) {
            ...
            $msg['body'] = __('messages.order_accepted', ['order_code' => $order->code,'sender_name'=>$user_sender->name]);
            $msg['order_status'] = 1;
            // $response = $this->notifyFcm($user_receiver, $msg);
            $response = $this->sendGCM($msg,$user_receiver->fcm_token);
        ...
        if (in_array('db', $to)) {
            ...
            $data['title'] = 'congrate';
            $data['params'] = ['order_code' => $order->code,'sender_name'=>$user_sender->name];
            $data['order_status'] = 2;
            $this->store($data);
        }
        app()->setLocale($this->locale);
    ...
    public function notifyOrderStatus($to = [], $params = [])
    {
        ...
        if ($params['status'] == 3) {
            $body = __('messages.order_on_way', ['order_code' => $order->code, 'sender_name'=>$user_sender->name]);
            $status = 3;
        } else {
            $body = __('messages.order_delivered', ['order_code' => $order->code, 'sender_name'=>$user_sender->name]);
            $status = 4;
        }
    ...
    public function notifyInvoiceStatus($to = [], $params = [])
    {
        ...
        $this->change_locale($user_receiver->id);
            ...
            $body = __('messages.invoice_paid', ['order_code' => $order->code,'sender_name'=>$user_sender->name]);
            $status = 2;
        ...
        if (in_array('fcm', $to)) {
            ...
            $msg['order'] = $order->id;
            $msg['type'] = 1;
            $msg['title'] = __('messages.invoice_paid');
            $msg['body'] = $body;
            $msg['order_status'] = $status;
            $response = $this->sendGCM($msg,$user_receiver->fcm_token);
        ...
        if (in_array('db', $to)) {
            ...
            $data['table_id'] = $order->id;
            $data['type'] = 1; // order 1 , 2 chat
            $data['title'] = 'invoice_paid';
            $data['data'] = 'invoice_paid';
            $data['params'] = ['order_code' => $order->code,'sender_name'=>$user_sender->name];
            $data['order_status'] = $status;
            $this->store($data);
        }
    ...
    public function notifychangePayment($to = [], $params = [])
    {
        ...
        if (in_array('fcm', $to)) {
            ...
            $msg['order'] = $order->id;
            $msg['type'] = 1;
            $msg['title'] = __('messages.change_payment' ,['sender_name' => $user_sender->name]);
            $msg['body'] = '';
            $msg['order_status'] = 3;
            // $response = $this->notifyFcm($user_receiver, $msg);
            $response = $this->sendGCM($msg,$user_receiver->fcm_token);
        ...
        if (in_array('db', $to)) {
            ...
            $data['params'] = ['sender_name'=>$user_sender->name];
            $data['order_status'] = 3;
            $this->store($data);
        }
        app()->setLocale($this->locale);
    ...
    public function notifyCancelOrder($to = [], $params = [])
    {
        ...
        $body = __('messages.order_calnceled', ['order_code' => $order->code,'sender_name'=>$user_sender->name]);
    ...
    public function notifyCancelOrderByAdmin($to = [], $params = [])
    {
        ...
        $order = Order::where('id', $params['order_id'])->select('id', 'user_id', 'shipper_id', 'code')->first();
          ...
          $body = __('messages.order_calnceled', ['order_code' => $order->code,'sender_name'=>'الادمن']);
          ...
          if (in_array('db', $to)) {
              ...
              $data['data'] = 'order_calnceled';
              $data['order_status'] = 5;
              $data['params'] = ['order_code' => $order->code,'sender_name'=>'الادمن'];
              $this->store($data);
          }
          app()->setLocale($this->locale);
        ...
        $body = __('messages.order_calnceled', ['order_code' => $order->code,'sender_name'=>'الأدمن']);
    ...
    public function notifyDeserveMore($to = [], $params = [])
    {
        ...
        $body = __('messages.order_deserve_more', ['order_code' => $order->code,'sender_name'=>$user_sender->name]);
    ...
    public function notifySubmitOffer($to = [], $params = [])
    {
        ...
        // get current item
        $order = Order::where('id', $params['order_id'])->select('id', 'user_id', 'shipper_id', 'code','status')->first();
    ...
    public function notifyDecreaseWallet($to = [], $params = [])
    {

        $receiver_id = $params['driver_id'];
        ...
        $body = __('messages.decrease_wallet_message', ['amount' => $params['amount'], 'user_name' => optional($user_sender)->name]);
        ...
        if (in_array('fcm', $to)) {
            ...
            $msg['order'] = $order->id;
            $msg['type'] = 7;
            $msg['title'] = __('messages.wallet_decrease');
            $msg['body'] = $body;
            // $response = $this->notifyFcm($user_receiver, $msg);
            $response = $this->sendGCM($msg,$user_receiver->fcm_token);
        ...
        if (in_array('db', $to)) {
            ...
            $data['table_id'] = $order->id;
            $data['type'] = 7; // order 1 , 2 chat
            $data['title'] = 'wallet_decrease';
            $data['data'] = 'decrease_wallet_message';
            $data['params'] = ['amount' => $params['amount'], 'user_name' => optional($user_sender)->name];
            $this->store($data);
        }
        app()->setLocale($this->locale);
    ...
    public function notifyErrorPayment($to = [], $params = [])
    {
        ...
        $body = __('messages.error_message');
        ...
        if (in_array('fcm', $to)) {
            ...
            $msg['order'] = $order->id;
            $msg['type'] = 13;
            $msg['title'] = __('messages.error_message');
            $msg['body'] = $body;
            // $response = $this->notifyFcm($user_receiver, $msg);
            $response = $this->sendGCM($msg,$user_receiver->fcm_token);
        ...
        if (in_array('db', $to)) {
            $data['user_sender_id'] = 1;
            $data['user_reciever_id'] = $user_receiver->id;
            $data['table_name'] = 'orders';
            $data['table_id'] = $order->id;
            $data['type'] = 13; // order 1 , 2 chat
            $data['title'] = 'error_message';
...
public function notifySucessPayment($to = [], $params = [])
{
    ...
    $body = __('messages.success_message');
    ...
    if($user_receiver){
        if (in_array('fcm', $to)) {
            ...
            $msg['order'] = $order->id;
            $msg['type'] = 13;
            $msg['title'] =  __('messages.success_message');
            $msg['body'] = $body;
            // $response = $this->notifyFcm($user_receiver, $msg);
            $response = $this->sendGCM($msg,$user_receiver->fcm_token);
        ...
        if (in_array('db', $to)) {
            ...
            $data['table_id'] = $order->id;
            $data['type'] = 13; // order 1 , 2 chat
            $data['title'] = 'success_message';
            $data['data'] = 'success_message';
            $data['params'] = ['order_code' => $order->code, 'user_name' => $user_receiver->name];
            $this->store($data);
        }
    }
...
public function notifySucessPaymentUser($to = [], $params = [])
{
    ...
    public function notifyChat($to = [], $params = [])
    {
        ...
        $body = __('messages.chat_body', ['order_code' => $order->code, 'user_name' => $user_sender->name]);
    ...
    public function getUnreadNotificationCountByUserId($userId)
    {
        return Notification::where('user_reciever_id', $userId)->Unread()->count();
    }
